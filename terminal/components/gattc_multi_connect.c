...
static const char remote_device_name[2][20] = {"ESP_GATTS_DEMO_a", "ESP_GATTS_DEMO_b"};
...
esp_gatt_if_t gattc_if1;
esp_gatt_if_t gattc_if2;
...
static void gattc_profile_a_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_OPEN_EVT:
        ...
        esp_log_buffer_hex(GATTC_TAG, p_data->open.remote_bda, sizeof(esp_bd_addr_t));
            ...
            if (memcmp(p_data->open.remote_bda, p_data->open.remote_bda, sizeof(esp_bd_addr_t)) == 0) {
                conn_device_a = true;
                ESP_LOGI(GATTC_TAG, "Device A connected");
            }
    ...
    case ESP_GATTC_WRITE_DESCR_EVT:
        ...
        uint8_t write_data[]="hello world1";
    ...
    case ESP_GATTC_WRITE_CHAR_EVT:
        if (p_data->write.status != ESP_GATT_OK){
            ...
            if (conn_device_a && conn_device_b) {
                ESP_LOGI(GATTC_TAG, "Both devices connected, stopping scan");
            } else {
                start_scan();
            }
...
static void gattc_profile_b_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_OPEN_EVT:
        ...
        esp_log_buffer_hex(GATTC_TAG, p_data->open.remote_bda, sizeof(esp_bd_addr_t));
            ...
            if (memcmp(p_data->open.remote_bda, p_data->open.remote_bda, sizeof(esp_bd_addr_t)) == 0) {
                conn_device_b = true;
                ESP_LOGI(GATTC_TAG, "Device B connected");
            }
    ...
    case ESP_GATTC_WRITE_DESCR_EVT:
        ...
        ESP_LOGI(GATTC_TAG, "write descr success");
        uint8_t write_char_data[35];
        uint8_t write_data[]="hello world2";
        for (int i = 0; i < sizeof(write_char_data); ++i)
        {
            write_char_data[i] = i % 256;
        }
