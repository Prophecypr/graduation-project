...
static char test_device_name[ESP_BLE_ADV_NAME_LEN_MAX] = "ESP_GATTS_DEMO_b";
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            handle_received_notification(param->write.value,param->write.len);
...
void handle_received_notification(uint8_t *data, uint16_t length) {
    // 在这里处理接收到的数据
    if (length == GATT_DATA) {
        client_mode=-1;
        // 假设数据是两个 8 位温湿度值
        ...
        latest_hum    = data[2];
        MQTT_cloud_data_forwarding(latest_mode + 20 , latest_temp , latest_hum);
        ESP_LOGI(GATTS_TAG, "Processed temperature data: %i.%i", latest_temp / 10 ,latest_temp % 10);
        ESP_LOGI(GATTS_TAG, "Processed humidity data: %i%%", latest_hum);
        // ESP_LOGI(GATTS_TAG, "%d,%d", WS2812_black,client_mode);
    } else {
        ESP_LOGW(GATTS_TAG, "Unexpected notification length: %d", length);
    }
}
