...
{
    uint8_t mac[6];
    esp_err_t ret = esp_wifi_get_mac(WIFI_IF_STA, mac);  // 获取STA模式的MAC地址
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "ESP32 MAC Address (STA): %02X:%02X:%02X:%02X:%02X:%02X",
                 mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
    } else {
        ESP_LOGE(TAG, "Failed to get MAC address, error code: %d", ret);
    }
}
