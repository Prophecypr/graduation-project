...
{
    const char *device_secret = "b81b6a53f026bedbac03f56b15c417af";
    const char *product_key = "hk8yVDm91aS";
    const char *device_name = "esp32c3_4g_gateway";
...
void MQTT_cloud_data_forwarding(int mode,int temperature,int humidity)
{
    ...
    "AT+MPUB=\"/hk8yVDm91aS/esp32c3_4g_gateway/user/temperature/update\",0,0,\"%i.%i\"\r", temperature/10,temperature%10);
    sendStringData(TAG,cmd_temperature);
    vTaskDelay(100 / portTICK_PERIOD_MS);
    snprintf(cmd_humidity, sizeof(cmd_humidity), 
    "AT+MPUB=\"/hk8yVDm91aS/esp32c3_4g_gateway/user/humidity/update\",0,0,\"%i\"\r", humidity);
    sendStringData(TAG,cmd_humidity);
    vTaskDelay(100 / portTICK_PERIOD_MS);
    snprintf(cmd_mode, sizeof(cmd_mode), 
    "AT+MPUB=\"/hk8yVDm91aS/esp32c3_4g_gateway/user/mode/update\",0,0,\"%i\"\r", mode);
...
int m100p_init()
{
    ...
    if (send_command_and_wait("AT+MSUB=\"/hk8yVDm91aS/esp32c3_4g_gateway/user/mode/update\",0\r")) {
